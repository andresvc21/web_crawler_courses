...
def format_for_csv(value):
    ...
    """Normalize values for CSV while avoiding data loss."""

    if isinstance(value, list):

        if all(isinstance(item, str) for item in value):

            return " | ".join(item.strip() for item in value if item)
...
with open('complete_genesys_courses_dataset.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    for course in merged_courses:
        ...
        writer.writerow([
            ...
            format_for_csv(course.get('course_outline', '')),

            format_for_csv(course.get('target_audience', []))
